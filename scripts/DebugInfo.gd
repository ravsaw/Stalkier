...
@onready var camera_label: Label = $Camera
...
func update_debug_info():
	...
	if camera_controller:
		var zoom = camera_controller.camera_2d.zoom.x
		var pos = camera_controller.global_position
		camera_label.text = "Camera: Zoom=%.2f Pos=(%.0f,%.0f)" % [zoom, pos.x, pos.y]
	else:
		camera_label.text = "Camera: N/A"
